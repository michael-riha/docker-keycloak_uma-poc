...
def check_bearer(request: Request):
    try:
        ...
        if e.status_code == 401:
            ...
            login_url = f"{auth_url}?response_type={response_type}&client_id={client_id}&redirect_uri={redirect_uri}&scope={client.scope}&state={state}&nonce={nonce_value}"
...
def get_discovery_response():
    client = OAuthClient()
    return requests.get(url=client.oidcDiscoveryUrl)
